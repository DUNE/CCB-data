...
CPU calculations are now made in HS23 units.  Calculations for the protoDUNE's and DUNE far detector are based on existing reconstruction and simulation experience.  We have much less experience with Near Detector codes so those estimates are less accurate.
...
The Analysis model is currently crude.  Analysis processing is modeled by scaling the reconstruction by a multiplicative factor {\tt AnalysisCPU} and extending the time by {\tt AnalysisExtend} = \configAnalysisExtend years.  Analysis use of disk is modeled by also extending disk use by the same amount.  
...
\caption{Assumptions about splits of Disk resources between the US, CERN and Global. Note that }
\label{tab:DiskSplit}
\end{table}
