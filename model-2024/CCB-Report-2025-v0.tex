...
The calculation starts with numbers of events from the timeline and uses those numbers, with CPU/storage times/event from table\ref{tab:DetectorParameters} to calculate the annual new storage and CPU use for data taken in that year.  Figure/Tables \ref{tab:DataEventsPerYear} and \ref{tab:SimEventsPerYear} show the assumptions for numbers of events/year. 
...
Once the data volume generated per year is determined, those data are given retention timed and an assumed number of copies. For popular datasets the number of disk copies should be two and disk copies are retained for two years (two reconstruction/simulation cycles). 
